...
export const readFileAsExcel = (data) => {
  ...
  // 2-1-2 利用 xlsx 插件来解析文件内容
  const wookbook = XLSX.read(data, { type: 'array' })
  // 2-1-3 获取第一个表格（sheet）的名称
  const firstSheetName = wookbook.SheetNames[0]
  // 2-1-4 获取第一个表格的数据
  const firstSheetData = wookbook.Sheets[firstSheetName]
  // 2-1-5 解析表头 数据
